...
def ridge(stim, resp, alpha, normalpha=False, logger=ridge_logger):
    """
    SVD-free ridge regression using closed-form solution:
        wt = (X^T X + alpha I)^(-1) X^T Y

    stim: (T, N)
    resp: (T, M)
    alpha: scalar OR array of shape (M,)
    """
    T, N = stim.shape
    _, M = resp.shape
    ...
    # Alpha expansion
    if isinstance(alpha, (float, int)):
        alpha = np.ones(M, dtype=stim.dtype) * alpha

    # Solve per voxel
    wt = np.zeros((N, M), dtype=stim.dtype)

    for i in range(M):
        A = XtX + np.eye(N, dtype=stim.dtype) * alpha[i]
        # Solve A w = XtY[:, i]
        wt[:, i] = np.linalg.solve(A, XtY[:, i])
...
def ridge_corr_pred(Rstim, Pstim, Rresp, Presp, valphas, normalpha=False,
                    use_corr=True, logger=ridge_logger, **kwargs):
    """
    SVD-free ridge correlation prediction.
    """
    ...
    XtX = Rstim.T @ Rstim            # (N, N)
    XtY = Rstim.T @ Rresp            # (N, M)

    # Allocate output
    corr = np.zeros(M, dtype=np.float32)

    zPresp = zs(Presp)

    # For each voxel, compute ridge weights separately
    for v in range(M):
        alpha = valphas[v]

        A = XtX + np.eye(N, dtype=Rstim.dtype) * alpha
        w = np.linalg.solve(A, XtY[:, v])     # (N,)

        pred = Pstim @ w                      # (TP,)

        if use_corr:
            corr[v] = np.mean(zs(pred) * zPresp[:, v])
        else:
            resvar = np.var(Presp[:, v] - pred)
            Rsq = 1 - resvar / Presp[:, v].var()
            corr[v] = np.sign(Rsq) * np.sqrt(abs(Rsq))
...
def ridge_corr(Rstim, Pstim, Rresp, Presp, alphas, normalpha=False,
               use_corr=True, logger=ridge_logger, **kwargs):
    """
    SVD-free ridge evaluation across many alphas.
    """
    ...
    for alpha in alphas:
        A = XtX + np.eye(N, dtype=Rstim.dtype) * alpha
        # Solve for all voxels: (N,N) x (N,M) -> (N,M)
        wt = np.linalg.solve(A, XtY)
        ...
        if use_corr:
            ...
            resvar = (Presp - pred).var(0)
            Rsq = 1 - (resvar / Presp.var(0))
            Rcorr = np.sign(Rsq) * np.sqrt(abs(Rsq))

        Rcorr[np.isnan(Rcorr)] = 0
        Rcorrs.append(Rcorr)

    return np.array(Rcorrs)
